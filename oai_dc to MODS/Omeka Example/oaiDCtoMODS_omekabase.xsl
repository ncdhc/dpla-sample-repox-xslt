...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:oai_dc='http://www.openarchives.org/OAI/2.0/oai_dc/' xmlns:dc="http://purl.org/dc/elements/1.1/" version="2.0" xmlns="http://www.loc.gov/mods/v3">
  ...
  <!-- This stylesheet processes standard dublin core fields from standard oai_dc OAI feeds. It assumes:
  ...
  <xsl:output omit-xml-declaration="yes" indent="yes"/>
      ...
      <!-- OMEKA OAI provides ref URLs and thumbnail URLs in dc:identifier -->
      <xsl:template match="dc:identifier" mode="locationurl">
        <xsl:if test="contains(.,'http')"> 
          <xsl:if test="contains(.,'/archive/files/')">
            <xsl:choose>
              <xsl:when test="contains(.,'.mp3') or contains(.,'.pdf') or contains(.,'.mov') or contains(.,'.mp3') or contains(.,'.aiff') or contains(.,'.midi') or contains(.,'.ra') or contains(.,'.wmv') or contains(.,'.mp4') or contains(.,'.qt') or contains(.,'.swf') or contains(.,'.avi') or contains(.,'.mpeg') or contains(.,'.war') or contains(.,'.txt') or contains(.,'.doc') or contains(.,'.ogg')">
                <!-- do nothing; this is a resource URL without a thumbnail -->
              </xsl:when>
              <xsl:otherwise>
                <!-- this is an image url that has a thumbnail -->
                <!-- OMEKA thumbnail pattern involves replacing /archive/files/ with /archive/square_thumbnails -->
                <xsl:variable name="thumburl" select="concat(replace(.,'/files/','/square_thumbnails/'),'.jpg')"/>
                <location><url access="preview"><xsl:value-of select="$thumburl"/></url></location> <!--OMEKA thumbnail url-->
              </xsl:otherwise>
            </xsl:choose>
          </xsl:if>
          <xsl:if test="contains(.,'/items/show/')">
            <location><url usage="primary display" access="object in context"><xsl:value-of select="normalize-space(.)"/></url></location> <!-- ref url-->     
          </xsl:if>     
        </xsl:if>
      </xsl:template>
